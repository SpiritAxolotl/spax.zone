...
function updateTime() {
  ...
  if (day >= 1 && day <= 4)
    ...
    if (time >= 60*7+30 && time < 60*(12+2)+30)
      ...
      if (time >= 60*10+45)
        if (time < 60*11+15)
          ...
          output = `Yo! I'm currently in school! But I'll be out in ${calcTimeUntil(time, 60*(12+2)+30)}!`;
      else
        //sets html for school (before lunch)
        output = `Yo! I'm currently in school! But I'll be having lunch in ${calcTimeUntil(time, 60*10+45)}!`;
    //if time is between 12:00am and 6:15am
      ...
      output = `Yo! I'm <i>technically</i> in school right now, but it hasn't started yet (but it <i>will</i> in ${calcTimeUntil(time, 60*7+30)}.)`;
    else
